                              ...
                              const AVFrame *rgba, AVFrame *pal8)
{
    ...
    AVPrivPaletteMapContext *map_ctx;
    ...
    map_ctx = avpriv_palette_map_init(pal, 128);
    if (!map_ctx) {
        av_free(argb_buf);
        return AVERROR(ENOMEM);
    }

    ret = avpriv_palette_map_apply(map_ctx,
                                pal8->data[0], pal8->linesize[0],
                                argb_buf, w * sizeof(uint32_t),
                                0, 0, w, h, s->dither, 0);

    /* Store the mapper's internal palette — it sorts entries during init,
     * so the indices returned by avpriv_palette_map_apply correspond to the
     * sorted order, not the original av_quantize_generate_palette order. */
    memcpy(pal8->data[1], avpriv_palette_map_get_palette(map_ctx), AVPALETTE_SIZE);

    avpriv_palette_map_uninit(&map_ctx);
    av_free(argb_buf);
    if (ret < 0)
        return ret;
