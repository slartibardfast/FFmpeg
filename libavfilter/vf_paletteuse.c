...
#include "libavutil/palettemap_internal.h"
#include "avfilter.h"
#include "filters.h"
#include "formats.h"
#include "framesync.h"
...
static const AVOption paletteuse_options[] = {
    { "dither", "select dithering mode", OFFSET(dither), AV_OPT_TYPE_INT, {.i64=FF_DITHERING_SIERRA2_4A}, 0, FF_NB_DITHERING-1, FLAGS, .unit = "dithering_mode" },
        { "bayer",           "ordered 8x8 bayer dithering (deterministic)",                            0, AV_OPT_TYPE_CONST, {.i64=FF_DITHERING_BAYER},           INT_MIN, INT_MAX, FLAGS, .unit = "dithering_mode" },
        { "heckbert",        "dithering as defined by Paul Heckbert in 1982 (simple error diffusion)", 0, AV_OPT_TYPE_CONST, {.i64=FF_DITHERING_HECKBERT},        INT_MIN, INT_MAX, FLAGS, .unit = "dithering_mode" },
        { "floyd_steinberg", "Floyd and Steingberg dithering (error diffusion)",                       0, AV_OPT_TYPE_CONST, {.i64=FF_DITHERING_FLOYD_STEINBERG}, INT_MIN, INT_MAX, FLAGS, .unit = "dithering_mode" },
        { "sierra2",         "Frankie Sierra dithering v2 (error diffusion)",                          0, AV_OPT_TYPE_CONST, {.i64=FF_DITHERING_SIERRA2},         INT_MIN, INT_MAX, FLAGS, .unit = "dithering_mode" },
        { "sierra2_4a",      "Frankie Sierra dithering v2 \"Lite\" (error diffusion)",                 0, AV_OPT_TYPE_CONST, {.i64=FF_DITHERING_SIERRA2_4A},      INT_MIN, INT_MAX, FLAGS, .unit = "dithering_mode" },
        { "sierra3",         "Frankie Sierra dithering v3 (error diffusion)",                          0, AV_OPT_TYPE_CONST, {.i64=FF_DITHERING_SIERRA3},         INT_MIN, INT_MAX, FLAGS, .unit = "dithering_mode" },
        { "burkes",          "Burkes dithering (error diffusion)",                                     0, AV_OPT_TYPE_CONST, {.i64=FF_DITHERING_BURKES},          INT_MIN, INT_MAX, FLAGS, .unit = "dithering_mode" },
        { "atkinson",        "Atkinson dithering by Bill Atkinson at Apple Computer (error diffusion)",0, AV_OPT_TYPE_CONST, {.i64=FF_DITHERING_ATKINSON},        INT_MIN, INT_MAX, FLAGS, .unit = "dithering_mode" },
...
static void disp_node(AVBPrint *buf,
                      const struct FFColorNode *map,
                      int parent_id, int node_id,
                      int depth)
{
    const struct FFColorNode *node = &map[node_id];
...
static int disp_tree(const struct FFColorNode *node, const char *fname)
{
    AVBPrint buf;
...
static int apply_palette(AVFilterLink *inlink, AVFrame *in, AVFrame **outf)
{
    ...
    ret = avpriv_palette_map_apply(s->pm, out->data[0], out->linesize[0],
    ...
    memcpy(out->data[1], avpriv_palette_map_get_palette(s->pm), AVPALETTE_SIZE);
    *outf = out;
    return 0;
}
...
static int load_palette(PaletteUseContext *s, const AVFrame *palette_frame)
{
    ...
    avpriv_palette_map_uninit(&s->pm);
    s->pm = avpriv_palette_map_init(palette, s->trans_thresh);
    if (!s->pm)
        return AVERROR(ENOMEM);

    if (s->dot_filename)
        disp_tree(avpriv_palette_map_get_nodes(s->pm), s->dot_filename);
