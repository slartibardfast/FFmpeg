...
static AVFrame *get_palette_frame(AVFilterContext *ctx)
{
    ...
    nb_colors = avpriv_mediancut_build_palette(s->mc);
    if (nb_colors < 0) {
    ...
    avpriv_mediancut_get_palette(s->mc, palette);
    nb_refs = avpriv_mediancut_nb_colors(s->mc);
 ...
 */
static int update_histogram_diff(AVPrivMedianCutContext *mc,
                                 const AVFrame *f1, const AVFrame *f2)
{
    ...
    for (y = 0; y < f1->height; y++) {
        ...
        for (x = 0; x < f1->width; x++) {
            if (p[x] == q[x])
                continue;
            ret = avpriv_mediancut_add_color(mc, p[x]);
            if (ret < 0)
                return ret;
            nb_diff_colors += ret;
        }
 ...
 */
static int update_histogram_frame(AVPrivMedianCutContext *mc, const AVFrame *f)
{
    int x, y, ret, nb_diff_colors = 0;
...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    if (s->stats_mode == STATS_MODE_DIFF_FRAMES) {
        ...
        s->prev_frame = in;
    } else if (s->stats_mode == STATS_MODE_SINGLE_FRAMES && avpriv_mediancut_nb_colors(s->mc) > 0) {
        AVFrame *out;
...
static int request_frame(AVFilterLink *outlink)
{
    ...
    if (r == AVERROR_EOF && !s->palette_pushed && avpriv_mediancut_nb_colors(s->mc) > 0
...
static int init(AVFilterContext *ctx)
{
    ...
    s->mc = avpriv_mediancut_alloc(s->max_colors - s->reserve_transparent);
    if (!s->mc)
