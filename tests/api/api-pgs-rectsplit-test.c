/*
 * PGS rect splitting (ff_sub_find_gap) test.
 *
...
static int test_no_gap(void)
{
    ...
    if (ff_sub_find_gap(buf, W * 4, W, 64, &gs, &ge) != 0) {
...
static int test_gap_too_small(void)
{
    ...
    if (ff_sub_find_gap(buf, W * 4, W, h, &gs, &ge) != 0) {
...
static int test_exact_threshold(void)
{
    ...
    if (ff_sub_find_gap(buf, W * 4, W, h, &gs, &ge) != 1) {
...
int main(void)
{
    ...
    printf("Testing ff_sub_find_gap...\n");
    errors += test_no_gap();
