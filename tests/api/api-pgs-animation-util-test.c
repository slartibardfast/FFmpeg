 ...
 *
 * Tests ff_sub_classify_change(), compute_alpha_ratio(), and
 * ff_sub_scale_alpha() via direct .c inclusion.
 *
                               ...
                               int w, int h)
{
    ...
    ref_sum = ff_sub_alpha_sum(ref_rgba, w, h, ref_ls);
    cur_sum = ff_sub_alpha_sum(cur_rgba, w, h, cur_ls);
...
static int test_classify_identical(void)
{
    ...
    enum FFSubChangeType ct;
    int errors = 0;

    fill_rgba(buf, 255, 0, 0, 200);
    ct = ff_sub_classify_change(buf, 10, 20, TEST_W, TEST_H, TEST_LS,
                                  buf, 10, 20, TEST_W, TEST_H, TEST_LS);
    if (ct != FF_SUB_CHANGE_NONE) {
...
static int test_classify_alpha(void)
{
    ...
    ct = ff_sub_classify_change(buf0, 10, 20, TEST_W, TEST_H, TEST_LS,
                                  buf1, 10, 20, TEST_W, TEST_H, TEST_LS);
    if (ct != FF_SUB_CHANGE_ALPHA) {
...
static int test_classify_content(void)
{
    ...
    ct = ff_sub_classify_change(buf0, 10, 20, 4, 4, TEST_LS,
                                  buf1, 10, 20, 8, 4, 32);
    if (ct != FF_SUB_CHANGE_CONTENT) {
...
static int test_classify_null(void)
{
    ...
    /* Both NULL -> NONE */
    ct = ff_sub_classify_change(NULL, 0, 0, 0, 0, 0,
                                  NULL, 0, 0, 0, 0, 0);
    if (ct != FF_SUB_CHANGE_NONE) {
...
static int test_ff_sub_scale_alpha(void)
{
    ...
    /* 50% alpha scaling */
    ff_sub_scale_alpha(src, dst, 4, 50);
    ...
    /* 0% alpha */
    ff_sub_scale_alpha(src, dst, 4, 0);
    if (((dst[0] >> 24) & 0xff) != 0) {
    ...
    /* 100% alpha */
    ff_sub_scale_alpha(src, dst, 4, 100);
...
int main(void)
{
    ...
    printf("Testing ff_sub_classify_change...\n");
    ...
    printf("Testing ff_sub_scale_alpha...\n");
    errors += test_ff_sub_scale_alpha();
