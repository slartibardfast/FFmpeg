...
int main(void)
{
    ...
    enum FFSubChangeType worst = FF_SUB_CHANGE_NONE;
    int n_changes = 0;
    ...
    alpha_t0   = ff_sub_alpha_sum(rgba0, w0, h0, ls0);
    peak_alpha = alpha_t0;
    ...
    for (int64_t t = start_ms + frame_ms; t <= start_ms + dur_ms;
        ...
        change = ff_sub_classify_change(
            rgba0, x0, y0, w0, h0, ls0,
            rgba,  sx, sy, sw, sh, ls);

        if (change > worst)
            worst = change;
        if (change != FF_SUB_CHANGE_NONE)
            n_changes++;

        if (rgba) {
            alpha = ff_sub_alpha_sum(rgba, sw, sh, ls);
            if (alpha > peak_alpha) {
                peak_alpha = alpha;
                peak_time  = t;
            }
                                         ...
                                         start_ms, dur_ms);
    {
        ...
        ff_sub_render_sample(render, start_ms,
                                         &rgba, &ls,
                                         &sx, &sy, &sw, &sh, NULL);
        alpha_t0 = rgba ? ff_sub_alpha_sum(rgba, sw, sh, ls) : 0;
        av_free(rgba);

        ff_sub_render_sample(render, 1000,
                                         &rgba, &ls,
                                         &sx, &sy, &sw, &sh, NULL);
        alpha_mid = rgba ? ff_sub_alpha_sum(rgba, sw, sh, ls) : 0;
        av_free(rgba);

        ff_sub_render_sample(render, dur_ms,
                                         &rgba, &ls,
                                         &sx, &sy, &sw, &sh, NULL);
        alpha_end = rgba ? ff_sub_alpha_sum(rgba, sw, sh, ls) : 0;
        av_free(rgba);
    }
    ...
    if (worst != FF_SUB_CHANGE_ALPHA) {
        fprintf(stderr, "FAIL: Expected ALPHA(2), got %d\n", worst);
        goto end;
    }
    printf("PASS: Fade classified as FF_SUB_CHANGE_ALPHA\n");
...

    {
        ...
        for (int64_t t = start_ms; t <= start_ms + dur_ms;
            ...
            ff_sub_scale_alpha(ref_pal, scaled, nb_colors, pct);
