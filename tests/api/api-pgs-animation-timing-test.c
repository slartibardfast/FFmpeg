...
int main(void)
{
    AVSubtitleRenderContext *render = NULL;
    const AVCodec *codec;
    ...
    render = avfilter_subtitle_render_alloc(1920, 1080);
    if (!render) {
        fprintf(stderr, "Render alloc failed (libass not available?)\n");
        return 77; /* FATE skip */
    }
    avfilter_subtitle_render_set_header(render, ass_header);
    ...
    avfilter_subtitle_render_init_event(render, fade_text,
                                         start_ms, dur_ms);

    avfilter_subtitle_render_sample(render, start_ms,
                                     &rgba0, &ls0,
                                     &x0, &y0, &w0, &h0, NULL);
    if (!rgba0) {
    ...
    for (int64_t t = start_ms + frame_ms; t <= start_ms + dur_ms;
        ...
        avfilter_subtitle_render_sample(render, t, &rgba, &ls,
                                         &sx, &sy, &sw, &sh, &dc);
                                         ...
                                         start_ms, dur_ms);
    {
        ...
        avfilter_subtitle_render_sample(render, 1000,
                                         &rgba, &ls,
                                         &sx, &sy, &sw, &sh, NULL);
        alpha_mid = rgba ? rgba_alpha_sum(rgba, sw, sh, ls) : 0;
        av_free(rgba);

        avfilter_subtitle_render_sample(render, dur_ms,
                                         &rgba, &ls,
                                         &sx, &sy, &sw, &sh, NULL);
    ...
    avfilter_subtitle_render_init_event(render, fade_text,
                                         start_ms, dur_ms);
    avfilter_subtitle_render_sample(render, peak_time,
                                     &peak_rgba, &ls0,
                                     &x0, &y0, &w0, &h0, NULL);
    if (!peak_rgba) {
    ...
    av_free(peak_rgba);
    av_free(ref_indices);
    avfilter_subtitle_render_freep(&render);
