...
typedef struct FFNeuQuantContext {
    int netsize;
...
static int colorimportance(int32_t alpha_internal)
{
    ...
    return 1 + av_clip_uintp2(a, 8);
}

static void init_network(FFNeuQuantContext *ctx)
{
...
static int contest(FFNeuQuantContext *ctx, const int32_t pixel[4])
{
...
static void altersingle(FFNeuQuantContext *ctx, int alpha, int i,
...
static void alterneigh(FFNeuQuantContext *ctx, int radius, int i,
                        const int32_t pixel[4])
{
...
static void unbias_network(FFNeuQuantContext *ctx)
{
    for (int i = 0; i < ctx->netsize; i++) {
        for (int j = 0; j < 4; j++) {
...
static void build_index(FFNeuQuantContext *ctx)
{
    ...
    for (int i = 0; i < ctx->netsize; i++) {
        int smallpos = i;
        int smallval = av_clip_uintp2(ctx->network[i][0] >> 8, 8);

        for (int j = i + 1; j < ctx->netsize; j++) {
            int val = av_clip_uintp2(ctx->network[j][0] >> 8, 8);
            if (val < smallval) {
                smallpos = j;
                smallval = val;
            }
...
FFNeuQuantContext *ff_neuquant_alloc(int netsize)
{
    FFNeuQuantContext *ctx;
...
void ff_neuquant_free(FFNeuQuantContext **pctx)
{
    if (pctx) {
        av_freep(pctx);
    }
...
int ff_neuquant_learn(FFNeuQuantContext *ctx, const uint8_t *rgba,
...
void ff_neuquant_get_palette(const FFNeuQuantContext *ctx, uint32_t *palette)
{
    for (int i = 0; i < ctx->netsize; i++) {
        struct FFLabColor c = {
            .L = ctx->network[i][0],
        ...
        int alpha = av_clip_uintp2((ctx->network[i][3] + 128) / 257, 8);
        palette[i] = (uint32_t)alpha << 24 | srgb;
    }
}

int ff_neuquant_is_trained(const FFNeuQuantContext *ctx)
{
    return ctx && ctx->trained;
}

int ff_neuquant_map_pixel(const FFNeuQuantContext *ctx,
