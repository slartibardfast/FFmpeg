...
struct color_ref {
    uint32_t color;
    struct FFLabColor lab;
    int64_t count;
};

struct range_box {
    uint32_t color;
    struct FFLabColor avg;
    int major_axis;
    int64_t weight;
    int64_t cut_score;
    int start;
...
static void reset_state(AVPrivMedianCutContext *ctx)
{
...
void avpriv_mediancut_free(AVPrivMedianCutContext **pctx)
{
    if (pctx && *pctx) {
        AVPrivMedianCutContext *ctx = *pctx;
        reset_state(ctx);
        av_freep(pctx);
    }
}
                       ...
                       int nb_pixels)
{
    ...
    for (int i = 0; i < nb_pixels; i++) {
        ...
        uint32_t color = ((uint32_t)p[3] << 24) |
                         ...
                         ((uint32_t)p[1] <<  8) |
                          (uint32_t)p[2];
        ret = avpriv_mediancut_add_color(ctx, color);
        if (ret < 0) {
            avpriv_mediancut_reset(ctx);
            return ret;
        }
    ...
    ret = avpriv_mediancut_build_palette(ctx);
    if (ret < 0)
        avpriv_mediancut_reset(ctx);
    return ret;
}
                           ...
                           uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    uint32_t color = ((uint32_t)a << 24) | ((uint32_t)r << 16) |
                     ((uint32_t)g << 8) | b;
    struct FFLabColor target = ff_srgb_u8_to_oklab_int(color);
    int best = 0;
