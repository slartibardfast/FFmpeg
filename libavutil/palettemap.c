...
struct AVPrivPaletteMapContext {
    struct FFCacheNode cache[FF_PALETTE_CACHE_SIZE];
    struct FFColorNode map[AVPALETTE_COUNT];
...
static av_always_inline int diff(const struct FFColorInfo *a,
                                 const struct FFColorInfo *b,
...
static struct FFColorInfo get_color_from_srgb(uint32_t srgb)
{
    const struct FFLabColor lab = ff_srgb_u8_to_oklab_int(srgb);
    struct FFColorInfo ret = {.srgb=srgb, .lab={lab.L, lab.a, lab.b}};
    return ret;
}
...
static void colormap_nearest_node(const struct FFColorNode *map,
                                  const int node_pos,
                                  const struct FFColorInfo *target,
                                  const int trans_thresh,
                                  struct nearest_color *nearest)
{
    const struct FFColorNode *kd = map + node_pos;
    int nearer_kd_id, further_kd_id;
    const struct FFColorInfo *current = &kd->c;
 ...
 */
static av_always_inline int color_get(AVPrivPaletteMapContext *ctx, uint32_t color)
{
    struct FFColorInfo clrinfo;
    const uint32_t hash = ff_lowbias32(color) & (FF_PALETTE_CACHE_SIZE - 1);
    struct FFCacheNode *node = &ctx->cache[hash];
    struct FFCachedColor *e;
...
static av_always_inline int get_dst_color_err(AVPrivPaletteMapContext *ctx,
                                              uint32_t c, int *er,
                                              int *eg, int *eb)
{
    uint32_t dstc;
...
static int set_frame_internal(AVPrivPaletteMapContext *ctx,
                          ...
                          int *component, const struct color_rect *box)
{
    ...
    for (int i = 0; i < AVPALETTE_COUNT; i++) {
        ...
        const struct FFLabColor lab = ff_srgb_u8_to_oklab_int(c);
...
static int colormap_insert(struct FFColorNode *map,
                           uint8_t *color_used,
                           int *nb_used,
...
static void load_colormap(AVPrivPaletteMapContext *ctx)
{
    int nb_used = 0;
...
{
    if (pctx && *pctx) {
        AVPrivPaletteMapContext *ctx = *pctx;
...
const struct FFColorNode *avpriv_palette_map_get_nodes(const AVPrivPaletteMapContext *ctx)
{
    if (!ctx)
        return NULL;
    return ctx->map;
