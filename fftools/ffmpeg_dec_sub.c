...
/* ------------------------------------------------------------------ */
/* Subtitle OCR (Tesseract wrapper)                                    */
/* ------------------------------------------------------------------ */
...
typedef struct SubtitleOCRContext {
    TessBaseAPI *tess;
    int          initialized;
} SubtitleOCRContext;

static SubtitleOCRContext *subtitle_ocr_alloc(void)
{
    SubtitleOCRContext *ctx;
...
static void subtitle_ocr_freep(SubtitleOCRContext **pctx)
{
    SubtitleOCRContext *ctx;
...
static int subtitle_ocr_init(SubtitleOCRContext *ctx,
                              const char *language,
                              const char *datapath)
{
    if (!ctx)
...
static int subtitle_ocr_set_pageseg_mode(SubtitleOCRContext *ctx,
                                          int mode)
{
    char buf[16];
...
static int subtitle_ocr_recognize(SubtitleOCRContext *ctx,
...
typedef struct SubtitleOCRContext SubtitleOCRContext;
...
static void subtitle_ocr_freep(SubtitleOCRContext **ctx)
{
    (void)ctx;
}
...
static int ensure_ocr_context(SubtitleDecContext *ctx, OutputStream *ost)
{
    ...
    ret = subtitle_ocr_init(ctx->ocr, lang, datapath);
    if (ret < 0) {
    ...
    if (ctx->pageseg_mode > 0) {
        ret = subtitle_ocr_set_pageseg_mode(ctx->ocr,
                                             ctx->pageseg_mode);
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR,
                    ...
                    AVSubtitle *sub, AVPacket *pkt)
{
    ...
    for (i = 0; i < sub->num_rects; i++) {
        ...
        /* Run OCR */
        ret = subtitle_ocr_recognize(ctx->ocr,
                                      gray, 1, gw, gw, gh,
                                      &text, &confidence);
        if (ret < 0) {
            av_free(gray);
        ...
        if ((!text || strlen(text) < 3) && rect->w > 100) {
            av_free(text);
            text = NULL;
            subtitle_ocr_set_pageseg_mode(ctx->ocr, 7);
            ret = subtitle_ocr_recognize(ctx->ocr,
                                          gray, 1, gw, gw, gh,
                                          &text, &confidence);
            subtitle_ocr_set_pageseg_mode(ctx->ocr, 6);
            if (ret < 0) {
                av_free(gray);
                return ret;
            }
